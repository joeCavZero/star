        ...
        if let Some(file) = self.base_file.clone() {
            ...
            if let Some(binary_target_file) = self.binary_destiny.clone() {
                ...
                if self.registers {
                    ...
                    debugger::message(&format!("zero ------------> 0b{:016b}", star.registers.zero));
                    debugger::message(&format!("a ---------------> 0b{:016b}", star.registers.a));
                    debugger::message(&format!("b ---------------> 0b{:016b}", star.registers.b));
                    debugger::message(&format!("c ---------------> 0b{:016b}", star.registers.c));
                    debugger::message(&format!("d ---------------> 0b{:016b}", star.registers.d));
                    debugger::message(&format!("e ---------------> 0b{:016b}", star.registers.e));
                    debugger::message(&format!("f ---------------> 0b{:016b}", star.registers.f));
                    debugger::message(&format!("g ---------------> 0b{:016b}", star.registers.g));
                    debugger::message(&format!("aux1 ------------> 0b{:016b}", star.registers.aux1));
                    debugger::message(&format!("aux2 ------------> 0b{:016b}", star.registers.aux2));
                    debugger::message(&format!("aux3 ------------> 0b{:016b}", star.registers.aux3));
                    debugger::message(&format!("carry -----------> 0b{:016b}", star.registers.carry));
                    debugger::message(&format!("high ------------> 0b{:016b}", star.registers.high));
                    debugger::message(&format!("low -------------> 0b{:016b}", star.registers.low));
                    debugger::message(&format!("return address --> 0b{:016b}", star.registers.return_address));
                    debugger::message(&format!("stack pointer ---> 0b{:016b}", star.registers.stack_pointer));
                    debugger::message(&format!("program counter -> 0b{:016b}", star.registers.program_counter));
                }
            }
